      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-700/70">
          ...
          <input
            ...
            className="w-full bg-transparent text-slate-100 text-base outline-none placeholder-slate-400"
          />
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-slate-950/40 border-b border-slate-800 text-xs text-slate-400">
          ...
          <button
            ...
            className="hover:text-sky-300 flex items-center gap-1 bg-slate-800/60 px-2 py-0.5 rounded border border-slate-700/50"
          >
          ...
          <button
            ...
            className="hover:text-rose-300 flex items-center gap-1 bg-slate-800/60 px-2 py-0.5 rounded border border-slate-700/50"
          >
            <Lock className="w-3 h-3 text-rose-400" /> Private Vault
          </button>
        </div>
          ...
          {filtered.length > 0 ? (
            filtered.map((item, idx) => (
              ...
              >
                ...
                <ExternalLink className="w-4 h-4 text-slate-400 opacity-60 flex-shrink-0" />
              </div>
            ))
          ) : (
            <div className="py-8 text-center text-sm text-slate-400">
              No matching bookmarks found for "<span className="text-slate-200 font-semibold">{query}</span>"
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-2 bg-slate-950/60 border-t border-slate-800 text-[11px] text-slate-400 flex items-center justify-between">
          ...
          <span className="flex items-center gap-1 text-sky-400 font-medium">
            <Command className="w-3 h-3" /> Nest 3.0
          </span>
        </div>
      </div>
    </div>
